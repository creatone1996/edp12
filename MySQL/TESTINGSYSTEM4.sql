...
JOIN DEPARTMENT AS B ON B.DEPARTMENTID = A.DEPARTMENTID;
...
-- CAU 3
SELECT * FROM `ACCOUNT` AS A
JOIN POSITION B ON B.POSITIONID = A.POSITIONID
WHERE POSITIONNAME = 'DEV';

-- CAU 4
SELECT *, count(A.DEPARTMENTID) AS SL FROM `ACCOUNT` AS A
JOIN DEPARTMENT AS B ON B.DEPARTMENTID = A.DEPARTMENTID
GROUP BY A.DEPARTMENTID HAVING COUNT(A.DEPARTMENTID) >=3;

-- CAU 5
SELECT D.QUESTIONID, COUNT(D.QUESTIONID) AS SL2 FROM QUESTION AS D
RIGHT JOIN EXAMQUESTION AS C ON D.QUESTIONID = C.QUESTIONID
GROUP BY D.QUESTIONID HAVING COUNT(D.QUESTIONID) = 
(SELECT MAX(SL2) 
FROM (SELECT D.QUESTIONID, COUNT(D.QUESTIONID) AS SL2 FROM QUESTION AS D
RIGHT JOIN EXAMQUESTION AS C ON D.QUESTIONID = C.QUESTIONID
GROUP BY D.QUESTIONID) AS ED);


-- CAU 6
SELECT QU.CATEGORYID, CA.CATEGORYNAME, COUNT(QU.CATEGORYID) AS SL3 FROM QUESTION AS QU
JOIN CATEGORYQUESTION AS CA ON CA.CATEGORYID = QU.CATEGORYID
GROUP BY QU.CATEGORYID HAVING COUNT(QU.CATEGORYID);

-- CAU 7
SELECT D.QUESTIONID,D.CONTENT, COUNT(D.QUESTIONID) AS SL2 
FROM QUESTION AS D
RIGHT JOIN EXAMQUESTION AS C ON D.QUESTIONID = C.QUESTIONID
GROUP BY D.QUESTIONID HAVING COUNT(D.QUESTIONID);

-- CAU 8
SELECT D.QUESTIONID, COUNT(D.QUESTIONID) AS SL2 FROM ANSWER AS C
JOIN QUESTION AS D ON D.QUESTIONID = C.QUESTIONID
GROUP BY D.QUESTIONID HAVING COUNT(D.QUESTIONID) = (SELECT max(SL2) FROM (SELECT D.QUESTIONID, COUNT(D.QUESTIONID) AS SL2 FROM ANSWER AS C
JOIN QUESTION AS D ON D.QUESTIONID = C.QUESTIONID GROUP BY D.QUESTIONID) AS QA);


-- CAU 9
SELECT GR.GROUPID, COUNT(GR.GROUPID) AS SL2 
FROM `ACCOUNT` AS AC
JOIN GROUPACCOUNT AS GR ON GR.ACCOUNTID = AC.ACCOUNTID
GROUP BY GR.GROUPID HAVING COUNT(GR.GROUPID);

-- CAU 10
SELECT A.POSITIONNAME, COUNT(B.POSITIONID) AS SL2 FROM `ACCOUNT` AS B
RIGHT JOIN `POSITION` AS A ON A.POSITIONID = B.POSITIONID
GROUP BY B.POSITIONID HAVING COUNT(B.POSITIONID) = 
(SELECT MIN(SL2) 
FROM (SELECT A.POSITIONID, COUNT(B.POSITIONID) AS SL2 FROM `ACCOUNT` AS B
RIGHT JOIN `POSITION` AS A ON A.POSITIONID = B.POSITIONID
GROUP BY B.POSITIONID) AS ED);

-- CAU 11
SELECT *, COUNT(A.ACCOUNTID)
FROM ((`ACCOUNT` AS A
JOIN DEPARTMENT AS B ON A.DEPARTMENTID = B.DEPARTMENTID)
JOIN POSITION AS C ON A.POSITIONID = C.POSITIONID)
GROUP BY A.DEPARTMENTID, A.POSITIONID;

-- CAU 12
SELECT * 
FROM (((QUESTION AS A
JOIN ANSWER AS B ON A.QUESTIONID = B.QUESTIONID)
JOIN TYPEQUESTION AS C ON A.TYPEID = C.TYPEID)
JOIN CATEGORYQUESTION AS D ON A.CATEGORYID = D.CATEGORYID);

-- CAU 13
SELECT B.TYPENAME, COUNT(A.TYPEID) AS SL2 
FROM QUESTION AS A
JOIN TYPEQUESTION AS B ON A.TYPEID = B.TYPEID
GROUP BY A.TYPEID HAVING COUNT(A.TYPEID);

-- CAU 14
SELECT *, COUNT(A.GROUPID) AS SL
FROM GROUPACCOUNT AS A
JOIN `GROUP` AS B ON A.GROUPID = B.GROUPID
GROUP BY A.GROUPID HAVING COUNT(A.GROUPID)=0;

SELECT *, COUNT(A.GROUPID) AS SL
FROM GROUPACCOUNT AS A
JOIN `GROUP` AS B ON A.GROUPID = B.GROUPID
GROUP BY A.GROUPID HAVING COUNT(A.GROUPID);

-- CAU 16
SELECT *, COUNT(A.QUESTIONID) AS SL
FROM ANSWER AS A
JOIN QUESTION AS B ON A.QUESTIONID = B.QUESTIONID
GROUP BY A.QUESTIONID HAVING COUNT(A.QUESTIONID) = 0;

SELECT *, COUNT(A.QUESTIONID) AS SL
FROM ANSWER AS A
JOIN QUESTION AS B ON A.QUESTIONID = B.QUESTIONID
GROUP BY A.QUESTIONID HAVING COUNT(A.QUESTIONID);
...
SELECT DISTINCT ACCOUNTID, GROUPID FROM 
(
SELECT * FROM GROUPACCOUNT
WHERE GROUPID = 1
UNION 
...
-- CAU 18
SELECT *, COUNT(GROUPID) FROM GROUPACCOUNT
GROUP BY GROUPID HAVING COUNT(GROUPID) > 5;

SELECT *, COUNT(GROUPID) FROM GROUPACCOUNT
GROUP BY GROUPID HAVING COUNT(GROUPID) > 7;
