...
-- nhập vào id phongf ban va in ra tên phòng ban
USE TestingSystem3;

DROP PROCEDURE IF EXISTS PHONGBAN;
DELIMITER $$
CREATE PROCEDURE PHONGBAN (IN in_NAME VARCHAR(50) )
BEGIN
SELECT A.* FROM `ACCOUNT` A JOIN
...
END$$
DELIMITER ;
 CALL PHONGBAN('Engineering');
 
 drop procedure if exists phongban2;
 DELIMITER $$
CREATE PROCEDURE PHONGBAN2()
BEGIN
SELECT groupid,
(SELECT groupname from `group` where groupid = ga.groupid) as groupname,
count(*) from groupaccount as ga
GROUP BY groupid;
...
DELIMITER ;
 ...
 drop procedure if exists phongban3;
 DELIMITER $$
CREATE PROCEDURE PHONGBAN3(out out_typeid MEDIUMINT)
BEGIN
...
DELIMITER ;
 ...
 begin
 SELECT q.* ,count(q.typeid) from typequestion as a
 join question as q on a.typeid = q.typeid
 GROUP BY q.typeid having q.createdate = month(current_date());
 end$$
 delimiter ;
 ...
 -- cau 6
  drop procedure if exists abc1;abc1
 delimiter $$
 CREATE PROCEDURE abc1 (IN IN_KYTU VARCHAR (50))
 begin
 SELECT A.USERNAME FROM `ACCOUNT` A
 WHERE A.FULLNAME LIKE concat('%',IN_KYTU,'%')
 UNION
 SELECT B.GROUPNAME FROM `GROUP` B
 WHERE B.GROUPNAME LIKE CONCAT('%',IN_KYTU,'%');
 end$$
 delimiter ;
...
END$$
DELIMITER ;
Call insert_Account_by_Email('var_Email','var_Fullname');

-- CAU 9
 drop procedure if exists abc2;
 delimiter $$
 CREATE PROCEDURE abc2 (IN IDEX TINYINT UNSIGNED)
 begin
 DELETE 
 FROM EXAMQUESTION WHERE EXAMID = IDEX;
 DELETE
 FROM EXAM WHERE EXAMID = IDEX;
 end$$
 delimiter ;
