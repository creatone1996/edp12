...
-- cau 2
CREATE OR REPLACE VIEW  SLNV AS
SELECT A.*, G.GROUPID, COUNT(A.ACCOUNTID) AS SL FROM `ACCOUNT` AS A
JOIN GROUPACCOUNT AS G ON G.ACCOUNTID= A.ACCOUNTID
GROUP BY A.ACCOUNTID HAVING COUNT(A.ACCOUNTID) = (SELECT MAX(SL) FROM (SELECT A.*, COUNT(A.ACCOUNTID) AS SL FROM `ACCOUNT` AS A
JOIN GROUPACCOUNT AS G ON G.ACCOUNTID= A.ACCOUNTID
GROUP BY A.ACCOUNTID) AD);


-- CAU 3
CREATE OR REPLACE VIEW CONTENT1 AS 
SELECT* , length(CONTENT)  FROM QUESTION
...
SELECT A.DEPARTMENTNAME, COUNT(B.DEPARTMENTID) AS SL2 FROM `ACCOUNT` AS B
RIGHT JOIN DEPARTMENT AS A ON A.DEPARTMENTID = B.DEPARTMENTID
GROUP BY B.DEPARTMENTID HAVING COUNT(B.DEPARTMENTID) = 
(SELECT MAX(SL2) 
FROM (SELECT A.DEPARTMENTNAME, COUNT(B.DEPARTMENTID) AS SL2 FROM `ACCOUNT` AS B
RIGHT JOIN DEPARTMENT AS A ON A.DEPARTMENTID = B.DEPARTMENTID
GROUP BY B.DEPARTMENTID) AS ED);

-- CAU 5
CREATE or REPLACE VIEW ERINA AS
SELECT AC.* FROM QUESTION AS Q
JOIN `ACCOUNT` AS AC ON Q.CREATORID = AC.ACCOUNTID
WHERE ac.fullname LIKE 'ABELARD %';
